...
function generateTwiMLResponse(sayText: string, audioUrl?: string, gatherNext: boolean = true): string {
  ...
  if (gatherNext) {
    twiml += '<Gather numDigits="1" action="/api/webhooks/voice/process" method="POST" finishOnKey="#"/>';
  } else {
    twiml += '<Hangup/>';
  }
...
async function processVoiceInput(state: VoiceState, userInput: string, detailer: any): Promise<{ response: string; newState: VoiceState; isComplete: boolean; audioUrl?: string }> {
  ...
  if (state.step === 'greeting' || state.step === 'confirmation' || isComplete) {
    try {
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Voice processing:', { from, to, digits, callSid });
    ...
    const { response, newState, isComplete, audioUrl } = await processVoiceInput(voiceState, digits || 'yes', detailer);
    ...
    const twiml = generateTwiMLResponse(response, audioUrl, !isComplete);
