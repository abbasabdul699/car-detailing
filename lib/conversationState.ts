  ...
  switch (context.state) {
    ...
    case 'awaiting_date':
      try {
        ...
        const generalAvailabilityQueryMatch = userMessage.match(/(?:what\s+are.*(?:avai?lable|available|avaiable)\s+(?:times|dates|appointments)|(?:avai?lable|available|avaiable)\s+(?:times|dates|appointments)|do\s+you\s+have\s+any\s+openings|show\s+me\s+your\s+availability|what\s+(?:times|dates)\s+do\s+you\s+have|when\s+are\s+you\s+(?:avai?lable|available|avaiable)|yeah\s+what\s+are.*(?:avai?lable|available|avaiable)|what\s+services\s+do\s+you\s+provide)/i);
