...
export default function AiAreaPhotosStep({ params }: { params: { detailerId: string } }) {
  ...
  const handleContinue = async () => {
    ...
    // 2. Upload images with the bookingId and collect URLs
    ...
    for (const file of areaPhotos) {
      ...
      const res = await fetch('/api/upload/booking-image', {
        method: 'POST',
        body: formData,
      });
      const data = await res.json();
    ...
    // 3. Navigate to review page, passing bookingId and image URLs
    router.push(
      `/book/${detailerId}/review?bookingId=${bookingId}&service=${encodeURIComponent(selectedServices.join(','))}&vehicle=${encodeURIComponent(vehicle ?? '')}&areaImages=${encodeURIComponent(JSON.stringify(uploadedImageUrls))}`
    );
  };
